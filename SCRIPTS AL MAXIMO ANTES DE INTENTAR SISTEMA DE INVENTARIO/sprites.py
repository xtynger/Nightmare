...
class Player(pg.sprite.Sprite):
	...
	def clock_timer(self):
		self.watch = (self.H, + self.M, + self.S) 
			...
			if self.m > self.mt:
				...
				if self.S > self.ST:
					...
					if self.M > self.MT:
						...
						if self.H > self.HT:
							self.H = 0
			#print('. %d' %(self.H), ":", + (self.M), ":", + (self.S), ":", + (self.m))
			print(self.watch)
	def hit(self):
		self.damaged = True
	...
	def animate(self):
		...
		#if self.walking and self.weapon == 'shotgun':
			#if now - self.last_update > 40:
				#self.last_update = now
				#self.current_frame = (self.current_frame + 1) % len(self.walking_shotgun)
				#bottom = self.rect.bottom
				#self.image = self.walking_shotgun[self.current_frame]
				#self.rect = self.image.get_rect()
				#self.latest = self.image   
				#print(self.current_frame)
