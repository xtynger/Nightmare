...
class Game:
    ...
    def load_data(self):
        ...
        ### PRUEBAS PARA ITEM DROP DE ENEMIGOS #########################
        #self.pow_img = {}
        #self.pow_img['shield'] = pg.image.load(path.join(img_dir, 'pow2.png')).convert()
        #self.pow_img['gun'] = pg.image.load(path.join(img_dir, 'pow1.png')).convert()
...
##################### ########## P R U E B A  P A R A   E X P L O S I O N E S ################ ################### ################### ################### 
        self.explosion_anim = {}
        self.explosion_anim['zombie'] = []
        self.explosion_anim['granade'] = []

        for i in range(9):
            self.filename = 'RE0{}.png'.format(i)
            self.img = pg.image.load(path.join(img_folder, self.filename)).convert()
            self.img.set_colorkey(BLACK)
            self.img_granade = pg.transform.scale(self.img, (50,50))
            self.explosion_anim['granade'].append(self.img)
            self.img_zombie = pg.transform.scale(self.img, (32,32))
            self.explosion_anim['zombie'].append(self.img_zombie)

            #self.filename = 'RE0{}.png'.format(i)
            #self.explosion_img = pg.image.load(path.join(img_folder, self.filename))
            #self.explosion_img.set_colorkey(WHITE) 
            #self.explosion_anim['zombie'].append(self.explosion_img)  

            #filename = 'shield0{}.png'.format(i)
            #img = pg.image.load(path.join(img_dir, filename))
            #bimg_ship = pg.transform.scale(img, (60, 60))
            #forcefield_anim['ship'].append(img)
       # self.explosion_animation = []
       # for img in EXPLOSION_IMAGE:
       #     self.explosion_animation.append(pg.image.load(path.join(img_folder, img)).convert_alpha())
...
##############################################################################################################################       
   ...
   #         if self.intro_message:
                ...
                #self.draw_text("This will do the job, lets do this.", self.extra_font, 20, WHITE, 200, HEIGHT - 60, align="w")  
    #            intro = pg.time.get_ticks() 
    #        if self.intro_message and 5000 < intro /2:
   ...
   # def ticking(self):
    ...
    def run(self):
        ...
        self.time_count = TIME_COUNT
        self.message_delay = MSG_DELAY
        self.day_count = DAYTIME
        self.night_count = NIGHTTIME 
        self.cycle = CYCLE_CLOCK
        ##############################################################################################
        #import time
        ...
        while self.playing:
            ...
            ##########################   TEMPORARY MESSAGES CLOCK LOOP WORKING #########################
            ...
            if self.day:
                self.night = False
                self.day_count += timer2.tick() 
                #print(self.day_count, 'day')
                if self.day_count  > self.cycle:
                    self.night = True                       ###### ORIGINAL CODED BY ME #########
                    self.night_count = 0                    #    $$$$$$$$$$$$$$$$$$$$$$$$$      #
                                                            #####################################
            ################# NIGHT CLOCK
            if self.night:
                self.day = False
                self.night_count += timer2.tick() 
                #print(self.night_count, 'night')
                if self.night_count  > self.cycle:
                    self.day = True
                    self.day_count = 0
...
####################################  B U L L E T S   H I T S   M O B S ##############################################################
        ...
        for hit in hits:
            ...
            hit.vel = vec(0, 0)
            self.player.score += 2 
            #expl = Explosion(hit.rect.center, 'zombie')
            #all_sprites.add(expl)
               #ESTA LINEA HACE QUE SE SUMEN PUNTOS AL SCORE##################################
...
################### T O    D R A W    W E A P O N  S L O T S  W H E N   P I C K   W E A P O N S  3/3 ####################
    ...
    def text_bar(self):
        ...
        #self.draw_text("", self.extra_font, 20, WHITE, 200, HEIGHT - 60, align="w")
    ...
    def draw(self):
        ...
        self.draw_text('{}'.format(str(self.player.health)), self.hud_font, 20, WHITE, 130, 5, align="n")
        self.draw_text('{}'.format(str(self.player.armor)), self.hud_font, 20, WHITE, 130, 35, align="n")
        #self.inventory()
...
######################  T O   M A K E   P A U S E   M E N U #############################################
        ...
        if self.text and self.paused:    ##############  T O    D R A W    M E N U    A N D    I N V E N T O R Y #########################
                ...
                #self.draw_text2('Details: {}'.format(str(WEAPONS[self.player.weapon]['description'])), self.hud_font, 10, WHITE, 250, 80, align="nw")
               # self.draw_text2("Paused", self.tittle_font, 40, GREEN , WIDTH / 2, HEIGHT / 2 - 120, align="center")
        ...
        ###if self.intro_message:
        ###    self.text_bar()
        ###    self.draw_text("Bueno bueno, parece que la cosa pora aca no pinta nada bien", self.extra_font, 20, WHITE, 200, HEIGHT - 60, align="w")
       ###     self.draw_text("Vamos a ir con cuidado, la zona esta infestada de caminantes", self.extra_font, 20, WHITE, 200, HEIGHT - 40, align="w")
        ###    self.draw_text("Lo primero sera encontrar un arma, perdi todo en el ultimo ataque", self.extra_font, 20, WHITE, 200, HEIGHT - 20, align="w")
            #self.draw_text("There is one of those zombies, i need to try killing one", self.extra_font, 20, WHITE, 200, HEIGHT - 60, align="w")
                #if timer > 4000:

       # if self.knife:
       #     self.text_bar()
      #      self.draw_text("Ok esto servira por ahora, veamos", self.extra_font, 20, WHITE, 200, HEIGHT - 60, align="w")       

      #  if self.rifle:
      #      self.temp_message()
      #      self.text_bar()
      #      self.draw_text("Mucho mejor, justo lo que necesitaba..", self.extra_font, 20, WHITE, 200, HEIGHT - 60, align="w") 

      #  if self.gun:
      #      self.text_bar()
      #      self.draw_text("Bueno, esto esta mucho mejor, con esto puedo estar mas seguro..", self.extra_font, 20, WHITE, 200, HEIGHT - 60, align="w")            

      #  if self.shotgun:
     #       self.text_bar()
      #      self.draw_text("Hell yeahh, potencia de fuego, esto hara maravillas contra esas cosas..", self.extra_font, 20, WHITE, 200, HEIGHT - 60, align="w")            

      #  if len(self.mobs) == 3: 
      #      self.text_bar()
      #      self.draw_text("Bien, resisten bastante bien pero pueden morir, hay que ir con cuidado...", self.extra_font, 20, WHITE, 200, HEIGHT - 60, align="w")
      #      self.draw_text("y encontrar a los demas...", self.extra_font, 20, WHITE, 200, HEIGHT - 40, align="w")
            #self.message_2

        #if len(self.mobs) == 2:
        #    self.text_bar()
        #    self.draw_text("ugh.. two less, two more to go...", self.extra_font, 20, WHITE, 200, HEIGHT - 60, align="w")
            #self.message_2

       # if len(self.mobs) <2:
        #    self.text_bar()
        #    self.draw_text("Come with me you bastard, you are the last one", self.extra_font, 20, WHITE, 200, HEIGHT - 60, align="w")
        #    #self.message_2
            #self.show_chat = True
            #if self.show_chat:
        ...
        if self.paused:
            ...
            self.screen.blit(self.inventory, (100 , 100 ), self.inventory_rect)  #dibujar cuadros transparentes
            #self.inventory.fill((20,20,20,150))
