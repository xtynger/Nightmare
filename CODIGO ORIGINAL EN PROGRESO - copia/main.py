...
def draw_player_armor(surf, x, y, pct):
    if pct < 0:
        pct = 0
    BAR_LENGHT = 100
    BAR_HEIGHT = 20
...
class Game:
    ...
    def load_data(self):
        ...
        #self.spritesheet1 = Spritesheet(path.join(img_folder, SPRITESHEET_IDDLE_K))
        #self.spritesheet2 = Spritesheet(path.join(img_folder, SPRITESHEET_MOVING_K))

        #self.next = pg.image.load(path.join(img_folder, NEXT_STAGE_IMAGE))
        ...
        self.dim_screen = pg.Surface(self.screen.get_size()).convert_alpha()
        self.dim_screen.fill((10, 10, 10, 100))
        self.dim_screen2 = pg.Surface(self.screen.get_size()).convert_alpha()
        self.dim_screen2.fill((1, 1, 1, 150))
        self.dim_screen3 = pg.Surface(self.screen.get_size()).convert_alpha()
        self.dim_screen3.fill((15, 15, 15, 230))
...
#################### C O D E   T O   S E T    I M A G E S   T O   B E   D R A W N, IT MUST INCLUDE ITS RECT TOO #############################################
        ...
        self.backpack_slot = pg.image.load(path.join(img_folder, BACKPACK)).convert_alpha()
        self.backpack_slot = pg.transform.scale(self.backpack_slot, (70, 70))
        self.backpack_rect = self.backpack_slot.get_rect()

        self.inventory  = pg.image.load(path.join(img_folder, INVENTORY)).convert_alpha()
        #self.inventory = pg.transform.scale(self.inventory, (600, 500))
...
##################### ################### ################### ################### ################### ################### ################### ################### 
    ...
    def zombie_spawn2(self):
        for tile_object in self.map.tmxdata.objects: #el parametro .objects es para usar esa capa especifica de objetos que creamos
            obj_center = vec(tile_object.x + tile_object.width / 2,
                             tile_object.y + tile_object.height / 2)
            #if tile_object.name == 'player': #ESTO INDICA DONDE DIBUJAR EL PLAYER
                #self.player = Player(self, obj_center.x, obj_center.y)
            if tile_object.name == 'zombie2': #ESTO INDICA DONDE DIBUJAR EL ENEMIGO
                Mob(self, obj_center.x, obj_center.y)
    ...
    def player_spawn2(self):
        for tile_object in self.map.tmxdata.objects: #el parametro .objects es para usar esa capa especifica de objetos que creamos
            obj_center = vec(tile_object.x + tile_object.width / 2,
                             tile_object.y + tile_object.height / 2)
            if tile_object.name == 'player2': #ESTO INDICA DONDE DIBUJAR EL PLAYER
                self.player = Player(self, obj_center.x, obj_center.y)
...
##########################################################################################################################################################################################
        ...
        for hit in hits:
            ...
            #draw_text()'text', 'font_name', 'size', 'color', 'x', 'y', and 'align'
            #Mob(self, obj_center.x, obj_center.y)

            if random() < 0.4:
                hit.health -= WEAPONS[self.player.weapon]['damage']  * 20
                hit.vel = vec(0, 0)         
                print(hit.health)
...
############################ T O    D R A W    W E A P O N  S L O T S  W H E N   P I C K   W E A P O N S  3/3 ####################
    ...
    def inventory(self):
        #self.screen.blit(self.inventory,(WIDTH - 100 , HEIGHT - 80 ), self.inventory_rect)  #dibujar cuadros transparentes
       # self.slots_img.fill((20,20,20,150))  #rellenar los cuadros
        self.screen.blit(self.inventory,(100 , 100 ), self.inventory_rect)
        #self.draw_text("BACKPACK", self.extra_font, 15, WHITE , WIDTH - 70, HEIGHT - 28, align="center")  #dibujar nombre del arma
        #self.draw_text("" + str(WEAPONS['medkit']['ammount']), self.tittle_font, 20, RED, WIDTH - 30 , HEIGHT - 35 , align="center")    

    def backpack(self):
        self.screen.blit(self.slots_img, (WIDTH - 100 , HEIGHT - 80 ), self.slots_rect)  #dibujar cuadros transparentes
        self.slots_img.fill((20,20,20,150))  #rellenar los cuadros
        self.screen.blit(self.backpack_slot,(WIDTH - 130 , HEIGHT - 120 ), self.backpack_rect)
        self.draw_text("BACKPACK", self.extra_font, 15, WHITE , WIDTH - 70, HEIGHT - 28, align="center")  #dibujar nombre del arma
        #self.draw_text("" + str(WEAPONS['medkit']['ammount']), self.tittle_font, 20, RED, WIDTH - 30 , HEIGHT - 35 , align="center")
...
##################################################################### CREADO POR MI
    def draw_player_exp(self, surf, x, y, pct):
        if pct < 0:
            pct = 0
        self.percent = 100
        #self.player.exp_cap = self.player.BAR_LENGHT
        #self.player.BAR_LENGHT = 450
        self.BAR_HEIGHT = 10
        self.fill = (pct / self.player.BAR_LENGHT) * self.player.BAR_LENGHT
        self.outline_rect = pg.Rect(x, y, self.player.BAR_LENGHT, self.BAR_HEIGHT)
        self.fill_rect = pg.Rect(x, y, self.fill, self.BAR_HEIGHT)
        self.col = YELLOW

        pg.draw.rect(surf, self.col, self.fill_rect)
        pg.draw.rect(surf, WHITE, self.outline_rect, 2)  




        ######################## ESTO FUE CREADO POR MI PARA MEDIR EL SISTEMA DE EXPERIENCIA
        #######################PARA PODER MODIFICAR CUALQUIER COSA, HAY QUE CONSIDERAR LOS\
        ################## VALORES EN PLAYER, SETTINGS, Y DRAW
    ...
    def draw(self):
        ...
        if self.player.exp > self.player.exp_cap:
            self.player.exp = 0
            self.player.level += 1
            self.player.exp_get -= 8
...
####################### VIDA DE MOBS, NOCHE, DEBUG, ETC, OTROS RELACIONADOS, ESCRIBIR ACA################
    ...
    ########### PARA DIBUJAR DAMAGE EN EL MOB, CREAR FUNCION Y EJECUTARLA AQUUI##########
        ...
        draw_player_armor(self.screen, 10,40, self.player.armor / PLAYER_ARMOR)

        self.draw_player_exp(self.screen, 150, 5, self.player.exp / PLAYER_EXP) ################################################ PRUBA PARA EXP BAR
        #draw_player_exp() missing 4 required positional arguments: 'surf', 'x', 'y', and 'pct'



        self.draw_text('score: {}'.format(str(self.player.score)), self.hud_font, 15, WHITE, WIDTH / 2, 20, align="n") #LINEA DE SCORE INVENTADA POR MI #################################################################
        self.draw_text('clip: {}'.format(str(WEAPONS[self.player.weapon]['charger_size'])), self.hud_font, 15, WHITE, 10, 100, align="nw")   #LINEA INVENTADA POR MI PARA DIBUJAR CONTEO DE BALAS ###############
        self.draw_text('Level: {}'.format(str(self.player.level)), self.hud_font, 15, WHITE, 10, 120, align="nw")   #LINEA INVENTADA POR MI PARA DIBUJAR CONTEO DE BALAS ###############
        self.draw_text('Zombies: {}'.format(len(self.mobs)), self.hud_font, 30, WHITE, WIDTH - 10, 10, align="ne")
        self.draw_text('EXP: {}'.format(str(self.player.exp)), self.hud_font, 15, WHITE, WIDTH / 2, 50, align="n")
        #self.draw_text('EXP: {}'.format(str(self.player.BAR_LENGHT)), self.hud_font, 10, BLACK, WIDTH / 2,3 , align="n")
#####################################################################################################################
...
######################  T O   M A K E   P A U S E   M E N U #############################################
        ...
        if self.paused:
            ...
            self.screen.blit(self.dim_screen3, (0,0))
...
###############################################################################################
    ...
    def events(self):
        ...
        for event in pg.event.get():
           ...
           # if event.type == MOUSEBUTTONDOWN:
                #print (event.button)
                #if self.player_img.get_rect().collidepoint(x, y):
                #self.player.shoot()
                #print('shooting')
    ...
    def next_level(self):
        self.waiting
        self.screen.blit(self.next, (0,0), self.next_rect)
        self.next.fill((0,0,0))
        self.draw_text("NEXT STAGE", self.extra_font, 99, WHITE , WIDTH / 2, HEIGHT / 2, align="center")
        self.draw_text("get ready and press a key", self.extra_font, 60, WHITE , WIDTH / 2 , HEIGHT / 2 + 80, align="center")
